...
use warnings;
use strict;
use lib qw( /home/ksieber/scripts/ /home/ksieber/perl5/lib/perl5/ /local/projects-t3/HLGT/scripts/lgtseek/lib/ /opt/lgtseek/lib/)
...
foreach my $input (@$inputs) {
    ...
    chomp($fn);
    my $name = defined $options{output_prefix} ? $options{output_prefix} : $fn;
...
sub help {
    ...
    ____/Submit to SGE-grid\\_____________________________________________________________________
        --Qsub|q=               <0|1> [0] 1= qsub the job to the grid.
          --threads|t=          # of threads to use for bwa sampe.
          --job_name=           
