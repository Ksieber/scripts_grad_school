...
if($m8==1){
	# Run Blast
	open(IN,"blastall -p blastn -e 1 -T F -d $options{db} -m8 -i $fasta |") || confess "Error: Unable to start blast fh: blastall -p blastn -e 1 -T F -d $options{db} -m8 -i $fasta\n";
	...
	my $in = Bio::SearchIO->new(
		-file => "blastall -p blastn -e 1 -T F -d $options{db} -i $fasta |",
		-format => 'blast',
		-best => '$bho',
...
$result->best_hits_only($bho);
$result->max_significance($evalue_cutoff);
	...
	while( my $hit = $result->next_hit ) {
		while( my $hsp = $hit->next_hsp ){
			if($hsp->evalue <= $evalue_cutoff ){
				if($bho==1 && $hsp->evalue <= $bho{*query_id})
					print 
