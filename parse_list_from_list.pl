...
if ( $options{help} ) {
    die "Help: This script will parse out the good or bad lines(ids etc) from --input_list.
		--input_list|I=
		--good_list=
		--bad_list=
		--output_dir|o=
...
close $ifh;
close $ofh;
print STDERR "Completed parsing: $input for lines from:" . "$lgtseek->{good_list}" . "$lgtseek->{bad_list}" . ".\n";
