...
if($options{dfam_file}){
    ...
    while(<$dfam_file>){
        chomp;
        my @f=split(/\t/,$_);                                                             
        next if($f[4] > 0.0001);                                                         ## Skip if the E-value is not below "Trusted-Cutoff (0.0001) May change to "Gathering Cutoff @ E-value of 20?
        my @hit = split(/\./,$f[15]);
        my $chr;
...
if($options{dfam_dir}){
    foreach my $bam (@bam_list){
        ...
        while(<$in>){
            ...
            if(!$current_chr{$line[2]}){                                                       ##If I haven't seen this chr yet
                $tree = IntervalTree->new();                                                   ##clear tree
                open(my $dfam, "<", "$dfam_dir/$line[2].Dfam_hits");                           ##open dfam hits file to build tree for the new chr
                while(<$dfam>){ 
                    chomp;
                    my @f=split(/\t/,$_);
                    if($f[9] =~/\+/){                                                             ## If on the positive strand
