...
if($options{krona}){
	open(my $krona_fh, "| cut -f2 | perl -ne 's/;/\\t/g; print;' | /home/ksieber/bin/ktImportText - -q -o $out_dir\/$out_pref\_krona.html") or confess "Error: Can't open krona output: $out_dir\/$out_pref\_krona.html because: $!";
	$OUTPUT->add($krona_fh);
}
