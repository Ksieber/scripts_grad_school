...
# }
    ...
    if ( $options->{M_only} ) {
        run_cmd(
            "bwa sampe $ref $output_prefix\.1.sai $output_prefix\.2.sai $file1 $file2 | samtools view -bhS -F0x4 - > $output_prefix\.bam",
            $log
        );
    }
    elsif ( $options->{MM_only} ) {
        run_cmd(
            "bwa sampe $ref $output_prefix\.1.sai $output_prefix\.2.sai $file1 $file2 | samtools view -bhS -F0xC - > $output_prefix\.bam",
            $log
        );
    }
    elsif ( $options->{sam_output} ) {
