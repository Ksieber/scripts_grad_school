...
foreach my $id (@ids) {
   if($options{batch}){
    ...
    my $efetch = Bio::DB::EUtilities->new(
                                          -db => 'nucleotide',
                                          -id => $id,             
                                          -rettype => 'FASTA',   ### Change to to change downloaded format. FASTA, gb 
                                          -retmode => 'text',
                                          );
    
    open OUT, ">$id.fa" or die "Unable to open $id\n";           ### May need to change the output suffix depending on the file being downloaded
