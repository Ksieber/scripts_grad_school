...
my %options = ();
my $results = GetOptions( \%options, 'genbank_file=s', 'genbank_dir=s', 'input|i=s', 'output_dir|o=s', 'samtools_path:s', 'help|?|h' ) || pod2usage();
...
sub read_genbank {
    ...
    while ( my $seq = $stream->next_seq() ) {
        ...
        foreach my $feat (@feats) {
            ...
            elsif ( $feat->has_tag('gene') and $feat->has_tag('product')) {  # this is a hack to use non-bacterial gbk gene name as locus tag.  
                my @vals  = $feat->get_tag_values('product');
