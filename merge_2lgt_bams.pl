...
if ( $options{split_bam1_cov} == 1 ) {
    ...
    @bam1_region_list = &bam2regions_of_coverage(
        {   bam     => $options{input},
            ref     => $options{ref1},
...
if ( $options{split_bam2_cov} == 1 ) {
    ...
    @bam2_region_list = &bam2regions_of_coverage(
        {   bam     => $options{bam2},
            ref     => $options{ref2},
            min_cov => $options{min_cov},
        }
...
sub bam2regions_of_coverage {
    my $opts = shift;
    open( my $infh, "-|", "samtools mpileup -f $opts->{ref} -A $opts->{bam}" )
        || confess "ERROR: Can't open: samtools mpileup $opts->{ref} -A $opts->{bam}\n";

    my $min_cov = defined( $opts->{min_cov} ) ? $opts->{min_cov} : "2";
    my @ret_region_list;
    ...
    while (<$infh>) {
        ...
        # print STDERR "Test:\t$chr\t$current_position\t$cov\n";
        $final_chr_check = $chr;
        if ( $cov >= $min_cov ) {
            ...
            for my $previous_chr ( keys %previous_position ) {
                if ( $chr !~ /$previous_chr/ ) {
                    ...
                    # print STDERR "Foo 1\n";
                    for my $previous_chr ( keys %previous_position ) {
                        push( @ret_region_list, "$previous_chr\:$start_position{$previous_chr}\-$previous_position{$previous_chr}" );
            ...
            # print STDERR Dumper(%start_position);
            # print STDERR "Foo 5:\n";
            for my $previous_chr ( keys %previous_position ) {
                print STDERR "Push: $previous_chr\:$start_position{$previous_chr}\-$previous_position{$previous_chr}\n";
                push( @ret_region_list, "$previous_chr\:$start_position{$previous_chr}\-$previous_position{$previous_chr}" );
                undef %start_position;
                undef %previous_position;
            }

            # print STDERR "Foo 5.1:\n";
            print STDERR Dumper(%start_position);
        }
    }
