...
if($options{help}){
	...
	--microbiome_reference=	Reference header to merge microbiome files on. 
	--help\n";
}
...
my $microbiome_reference = $options{microbiome_reference} ? $options{microbiome_reference} : "/local/projects-t3/HLGT/references/bacteria/final.bam_header";
