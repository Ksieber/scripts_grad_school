...
=head1  SYNOPSIS
    ...
    snp_gene_overlap_finder.pl 
        [--genbank_file=/a/genbank_file.gbk || --genbank_dir=/a/dir/of/gbks/]
        --input|i=			VCF file
...
if( $options{'help'} ){
    confess "This script will find the genes that lgt reads overlap with.
    --input|i=            <VCF>
    --genbank_file=		<Gen Bank file to pull Gene cords from>     
    --genbank_dir=  	<Directory with GB files, will find correct GB file if avail>
...
&read_genbank();
# Build a tree cords for genes
print STDERR "Building tree . . .\n";
$tree->buildTree();
# Find each overlap for reads in VCF file
print STDERR "Processing input: $options{input} . . .\n";
&read_input();

sub get_ref_id {
    my @lines = `$samtoolsbin view -H $options{input}`;
    if($?) {
        confess "Had a problem $?\n";
    }
    my $id;
    map {
        if(/^\@SQ\s+SN:gi\|\d+\|\w+\|(\S+)\|/) {
    ...
    if(!$id) {
        confess "Unable to pull the id from $options{input}\n";
    }
    return $id;
}
...
sub get_gb_file {
    ...
    open OUT, ">$options{output_dir}/$id.gbk" or confess "Unable to open $options{output_dir}/$id.gbk\n";
...
sub read_bam {
    open(my $handle,"<", "$options{input}") || confess "Error: Can't open "
