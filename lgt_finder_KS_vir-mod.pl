...
sub _process_overall {
    ...
    while(<IN2>) {
        ...
        if(!$filtered_clones->{$hit->{template_id}}) {
            ...
            $hit->{genera} = $genera;
            my $lca = $traces_by_template->{$hit->{template_id}}->{$dir}->{hit}->{lca} ?$traces_by_template->{$hit->{template_id}}->{$dir}->{hit}->{lca} : $hit->{lineage};
...
sub filter_genera {
    ...
    if($genera_to_ignore->{$genera}) {
        print STDERR "Filtering because of $genera\n";
        $retval = 1;
    }
    return $retval;
}
       ...
       print STDERR "Processing $clone\n";
        if(!$filtered_clones->{$clone}) {
            ...
            if($forward && $reverse) {
                ...
                push(@$fwd_traces, join("\t",($fhit->{trace_id},$fhit->{evalue},$fhit->{align_len},$fhit->{lca})));
                ...
                push(@$rev_traces, join("\t",($rhit->{trace_id},$rhit->{evalue},$rhit->{align_len},$rhit->{lca})));
    	        ...
    	        if($options{vir}){
            	    ...
            	    elsif($reverse =~ /Homo/ && $forward =~ /Human/) {
                	    print "$fline\n$rline\n";
                    	my @outfields = ($clone,$forward,$reverse,join(",",@$fwd_traces),join(",",@$rev_traces));
                    	if($options{append_links}) {
                       	 push(@outfields, ("driley-lx:8080/blast/$clone\_F.html","driley-lx:8080/blast/$clone\_R.html"));
