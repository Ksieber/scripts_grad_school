...
open my $out3, ">$overallfile" or die "Unable to open overall output $overallfile because: $!\n";
...
if($options{lineage1} && $options{lineage2}) {
    ...
    if(!$options{output1} && $options{output_dir}) {
        $out2file = $options{output_dir}."/$name\_lineage2.out";
    }
